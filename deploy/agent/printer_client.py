...
UPLOAD_VERIFY_TRIES = 5
UPLOAD_VERIFY_DELAY_SEC = 0.7
...
def _verify_uploaded_file(ftps, filename: str, expected_size: int) -> None:
    last_error = None

    for _ in range(UPLOAD_VERIFY_TRIES):
        can_fallback_to_listing = False
        try:
            ftps.voidcmd("TYPE I")
            remote_size = ftps.size(filename)
            if remote_size == expected_size:
                return
            last_error = RuntimeError(f"remote size {remote_size}, expected {expected_size}")
            if remote_size is None:
                can_fallback_to_listing = True
        except Exception as exc:
            last_error = exc
            can_fallback_to_listing = True

        if can_fallback_to_listing:
            try:
                names = {os.path.basename(str(name)) for name in ftps.nlst()}
                if filename in names:
                    return
                last_error = RuntimeError(f"{filename} is not present in remote file list")
            except Exception as exc:
                last_error = exc

        time.sleep(UPLOAD_VERIFY_DELAY_SEC)

    raise RuntimeError(f"uploaded file verification failed for {filename}: {last_error}")
        ...
        try:
            ...
            def emit_progress(percent):
                if not progress_callback:
                    return
                try:
                    progress_callback(round(min(max(percent, 0.0), 100.0), 1), bytes_sent, filesize)
                except Exception as exc:
                    print(f"[{host}] upload progress callback failed: {exc}", flush=True)

            def handle_block(block):
                nonlocal bytes_sent, last_update, last_progress_update
                bytes_sent += len(block)
                now = time.time()
                percent = bytes_sent / filesize * 100 if filesize else 100.0
                if now - last_progress_update > 1 or bytes_sent >= filesize:
            ...
            _verify_uploaded_file(ftps, filename, filesize)
            emit_progress(100.0)
